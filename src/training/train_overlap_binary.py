...
def build_query_global_to_city_place_map(train_dataset, train_cities_list):
    ...
    Construye un diccionario:
        ...
        qidx_global -> (city, query_place_id)

    donde query_place_id coincide con la carpeta creada por calcular_overlap_vggt3.py:

        OVERLAP_ROOT / city / place_<query_place_id>
    """
    ...
    for city in train_cities_list:
        ...
        if "query_place_id" not in pack:
            raise KeyError(
                f"{positives_file} no contiene query_place_id. "
                "No se pueden cargar correctamente los overlaps VGGT."
            )

        query_paths_pack = np.asarray(pack["query_paths"])
        query_place_ids = np.asarray(pack["query_place_id"])

        qstem_to_place_id = {}

        for i, q_path in enumerate(query_paths_pack):
            q_stem = Path(center_path(q_path)).stem
            place_id = str(query_place_ids[i])
            qstem_to_place_id[q_stem] = place_id
        ...
        print(f"{city}: queries mapeadas a query_place_id = {matches_city}")
...
def msls_triplet_loss_with_overlap(
    ...
    """
    Compute the binary overlap-weighted Triplet Loss.

    A query-positive pair receives:
        weight = 0 if overlap == 0
        weight = 1 if overlap > 0
    ...
    Args:
        ...
        positive_overlap: Tensor of shape [B] containing the
            visual overlap between each query-positive pair.

        margin: Triplet Loss margin.

    Returns:
        loss: Mean binary overlap-weighted Triplet Loss.
        overlap_weight: Binary weight assigned to each triplet.
    """
    ...
    overlap_weight = (positive_overlap > 0.0).float()
...
def validate_vpr_citywise(
    ...
    Después se calcula una métrica global ponderada por el número de queries válidas.
    """
...
OUTPUT_DIR = Path(os.environ.get("OUTPUT_OVERLAP_BINARIO2_ROOT", PROJECT_ROOT / "outputs" / "binary" ))
...
for current_epoch in range(start_epoch, max_epochs + 1):
    ...
    torch.save({
       ...
       "val_metrics": {
        ...
        "uses_overlap": True,
        "overlap_experiment": "binary_overlap_mask",
        "overlap_loss": "binary_overlap_weight * relu(d_pos - d_neg + margin)",
        "binary_overlap_weight": True,
        "overlap_scale": 1.0,
        "overlap_scale_used": False,
